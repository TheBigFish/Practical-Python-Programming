...
- 只有 writable 返回 true 的 socket 才会被 poll
- handle_close dispatcher 已经实现，可以不实现
- 获取完所有的数据，连接关闭，dispatcher.recv 读取长度为 0，调用 handle_close，调用 dispatcher.close，从 poll 的 map 中删除 socket，map 为空时，asyncore.loop 退出循环
